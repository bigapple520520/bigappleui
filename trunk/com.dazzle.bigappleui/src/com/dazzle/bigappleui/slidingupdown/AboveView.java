...
public class AboveView extends ViewGroup {
    ...
    private int getDestByWhichScreenToGo(int which) {
        ...
        switch (which) {
        case LOCATION_UP:
            if (mode == SlidingUpDown.MODE_UP_DOWN || mode == SlidingUpDown.MODE_UP) {
                dest = aboveHeight;
            }
            break;
        case LOCATION_DOWN:
            if (mode == SlidingUpDown.MODE_UP_DOWN || mode == SlidingUpDown.MODE_DOWN) {
                dest = -aboveHeight;
            }
            break;
        }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if (null != slidingUpDown.getOpenPercentListener()) {
            slidingUpDown.getOpenPercentListener().openPercent(getPercentOpen());
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (mode == SlidingUpDown.MODE_UP_DOWN || (mode == SlidingUpDown.MODE_DOWN && deltaY < 0)
                    || (mode == SlidingUpDown.MODE_UP && deltaY > 0) || isLocationUp() && deltaY < 0
                    || isLocationDown() && deltaY > 0) {
                if (isLocationMiddle()) {
                    scrollBy(0, deltaY / 2);
                }
                else {
